...
wd.loc <- 'C:/Users/tfullman/Dropbox (TWS)/Tim/Arctic Landscape/Landscape Identification'
gis.loc <- 'C:/Users/tfullman/Dropbox (TWS)/Tim/Research/GIS'
...
tibble(
  base_path =
    gis.loc,
  rel_path = 
    c(rep('/Study Area', times = 3),
      rep('/Tim Created/Arctic_Landscape_TWS', times = 8),
      '/Tim Created/Work/NPRA',
      '/Tim Created/Representation',
      '/Mapping/NorthSlope_Infrastructure_NSSI/nsinfra_v10_09032021/NSInfra_09032021_shp',
      '/Mapping/Ambler',
      '/Mapping/akecoregions.tar/Ecoregions_level_3',
      '/Mapping/CountriesofWorld'),
  file =
      ## AK state boundary 
  ...
  ## getting returned.
  pwalk(
    function(base_path, rel_path, file, name, ...){
      ## Identify the full path of the shapefile
      str_c(base_path, rel_path, '/', file, '.shp') %>% 
        ## Load the shapefile
        st_read() %>% 
...
ak_cities <- 
  ## Load the shapefile
  st_read(paste0(gis.loc, '/From Ryan/cities/citiesx020.shp')) %>% 
...
hillshade <- raster::raster(paste0(gis.loc, '/From Ryan/AK_Development/statehillutm')) %>% 
